...
| `eval_queue.sh` | The GPU order after the facedrop trial — campaign 18 regen, stress DDIM-50, `downstream_utility`, stress DDIM-200 — and then it stops. Two waits, neither a timeout: `facedrop_post.sh` must have logged its own gate (two generation jobs on one 121 GB unified pool is how a CUDA allocation fails while `free` still reports tens of GB), and `runs/campaigns/regen_go` must exist, a shell fragment naming `CKPT_RUN` and `SAMPLER`. Those two come from the trial table, which does not exist when the queue is armed. It never creates `decoder_ft_go`: that gate is the author's go-ahead, not an ordering device. |
