...
| `diffusion/sampler.py` (clamp) | `VolumeGenerator.clamp_porosity`, ON by default and required for anything gated, holds a porosity request inside [POR_MIN, POR_MAX] before conditioning. Assessment 13 turns it off, and the manifest records `porosity_clamped` per case. The clamp is NOT symmetric: cond_por is log(phi + 1e-3), so at the bottom 0.000/0.001/0.002 all collapse to one request with it on, while at the top 0.150 is only +0.26 sd beyond POR_MAX. |
...
| `diffusion/sampler.py` | `DDIMSampler` (nested-CFG `predict_out`, `sample_batch`) and `VolumeGenerator` — ONE generation path: hybrid chunked joint denoising. Chunks of `chunk_tiles` 64-voxel tiles in raster order; inside a chunk overlapping `window_stride` windows jointly denoise its latent canvas with cosine-weighted ε fusion; each window's six faces come from the chunk canvas at `t`, a finished chunk re-noised to `t`, OOB, or UNKNOWN. `neighbour_mode` selects WHERE those faces come from: `"canvas"` is that production path, `"unknown"` makes every in-bounds face the CFG null (the ldm05 joint sampler) and `"reference"` reads them all from a `reference_latents` canvas of real encoded material (teacher forcing); the last two exist only for eval_v4's `assembly_modes` and OOB is kept in all three, because the canvas edge is geometry and not neighbour content. Decode is overlapped at `decode_stride` and blends grey + 3-class logits with a tapered window before the argmax. The nested CFG combines RAW network outputs (ε or v — an affine combination commutes with the affine v↔ε map at fixed `t`) and `ddim_step` converts once; `rescale_guidance` is the optional Lin §3.4 over-exposure correction, off unless `guidance.cfg_rescale > 0`. A window's requested porosity is `window_tile_mean` — the tile-grid field averaged over the window's voxel footprint, volume-weighted, on RAW φ before the clamp and `porosity_to_cond`. With no `material_map` given, the default one is the EXACT specimen-box/latent-cell intersection fraction — the product of the three per-axis overlaps in closed form, so a surface cell the box crosses carries its true fraction instead of being rounded to 1 or 0. Every random draw is `region_noise_field` — one canvas-sized field per draw (the initial canvas once, the finished-chunk re-noising at every timestep), rolled by `request_offset` so the noise is anchored to the REQUEST and not to the canvas; translating a request inside a bigger canvas therefore translates its noise with it, which is what the assembly assessment needs to hold fixed. Also `window_origins`, `window_weight`, `theta_from_layup` and `seam_discontinuity` (per-axis period, so window and chunk planes are reported separately against one interior baseline).  `chunk_overlap` (voxels, default 0 = unchanged) makes the chunks ADVANCE by `size - overlap` instead of tiling: the strip a chunk shares with the one before it is already solved, so it is pinned RePaint-style — re-noised to t and pasted each step, and restored exactly at the end — while the new chunk's window grid covers it. `chunk_overlap_pinned` says how much of the overlap is HELD at the predecessor's value while the successor denoises (None = all of it, which is what the first version did and is why it only half worked: pinning the whole strip pastes the predecessor's depleted rim into the successor's windows at every step, so the successor is steered to reproduce it and a blend mixes depleted with depleted). A shorter pin keeps only the predecessor's pore-normal INTERIOR fixed and frees its rim to be redrawn against a healthy trailing context. `chunk_overlap_write` (`blend` / `pin` / `successor`) then writes the free part as a cosine blend of the two chunks' own outputs, `chunk_blend_weight` ramping from 1 where the solved content is window INTERIOR to 0 where it is rim — so each side's rim is replaced by the other side's interior, which is what the chunk-plane porosity band needs. Pinning ALONE does not remove the band (the depleted voxels are the finished chunk's own rim, and pinning is what keeps them); `chunk_overlap_blend=False` keeps that variant runnable to show it. `_chunk_ranges` therefore returns LATENT CELL ranges, not tile ranges: a useful overlap is half a tile, because it must be a multiple of `window_stride` (32) and not of the 64-voxel tile.  `drop_neighbours_when_mixed` (default False) marks a window's EXISTS faces UNKNOWN when its set is MIXED — some faces solved, the face toward the next chunk not — which is where the band is deepest. That is `s_nb=0` per window, and at `s_por=1` it is algebraically `out_por`, so it costs ONE model call; a global `s_nb` change instead leaves the unguided special case (`s_por==1 and s_nb==1`) and pays three forward passes per window. OOB faces are left alone: the canvas edge is geometry, not neighbour content, and a literal global `s_nb=0` would tell an edge window the volume continues. |
...
### baselines (published methods, re-implemented to be compared)

Two prior methods, so ldm06's numbers have something other than a real floor to
be read against. Neither is conditional, so both are scored ONLY on the
request-free metrics — a porosity error is not defined for a generator that
cannot be asked for a porosity.

| Path | What it does |
|---|---|
| `baselines/slicegan/networks.py` | `Generator3D` (13.38M) + `Critic2D` and the paper's own geometry. The obvious reading of the paper is wrong: the padding schedule makes the latent-to-volume map AFFINE, `out = 32n - 64`, not a multiple, and reading it as a multiple produced a 34-cubed volume from a 4-cubed latent the first time. `latent_for_shape` / `shape_for_latent` are that relation as inverses and refuse a shape the generator cannot make. `volume_to_slices` cuts a volume into the 2-D slices the three critics see. |
| `baselines/slicegan/data.py` | `build_slice_bank` holds 200k real slices in RAM, balanced across the three axes, so training never touches the patch store again after the bank is built — the store is 272 GB and streaming it beside a training run deadlocked one. `to_critic_batch` puts real slices in the generator's own output space. |
| `baselines/slicegan/train.py` | WGAN-GP with one 3-D generator against three 2-D critics, and the per-axis losses logged apart: the three critics are the only place anisotropy is visible during training. |
| `baselines/ldm_phi_only/networks.py` | `PhiOnlyDenoiser` — the porous-rock recipe (Naiff et al. 2026: scalar-porosity-conditioned latent diffusion) on our data. It SUBCLASSES `UNet3DDenoiser` rather than copying it, so the encoder, bottleneck, decoder, output head, timestep embedding, porosity MLP and learned null are not merely equivalent to ldm06's — they are ldm06's, built by the same code from the same config, and cannot drift. Only the stem narrows (155 input channels to 8) and the depth, face-distance and neighbour-pooling MLPs go: 81.67 M against ldm06's 83.00 M, **98.4 %, so capacity is not the variable**. The forward signature is ldm06's exactly and the unused arguments are accepted and ignored, which makes it a drop-in at every call site — the engine, both sampler paths and eval_v4 are unchanged. `s_nb` is inert on it by construction (the guided sampler's third pass differs only in neighbour inputs), so the cfg assessment runs the s_por arm alone. |
| `baselines/ddpm3d/networks.py` | `UNet3DPixel` (25.33M) — a DDPM in PIXEL space, the control for "is the latent the thing that makes ldm06 work?" |
| `baselines/ddpm3d/data.py` | `PatchVolumes` streams 64-cubed real patches; `decode_sample` maps the model's output back to grey + 3-class label. |
| `baselines/ddpm3d/train.py` | `DDPMConfig` (T=1000, v-objective, zero-terminal-SNR), `sample_window` and `sample_volume`, which fuses overlapped windows with a Tukey taper because a pixel-space model has no chunk machinery to inherit. |
...
| `eval_v4/metrics.py` | Every measurement: phase fractions, porosity error and the 0.005 gate, failure flags and degenerate cells, the WITHIN-volume local fit (plus `pooled_dose_fit`, named `..._not_obedience`), both seam periods via the sampler's own `seam_discontinuity`, pore Dice and the chunk-plane slab, the grey air detector (constants and calibration imported from `_eval_v3`, never forked), cross-head disagreement, layup recovery through the T-I readers, geometry Dice, the campaign-08 layup floor / VAE tile-decode control read from their `results.json`, and the suite's one spatial-correlation pair — `lag_correlation` (pooled paired-Pearson per lag, campaign-01 T-D's estimator) with `correlation_length_1_over_e` (the 1/e crossing, interpolated between lags), used by both `field_stats` and the surface floor. `chunk_blocks` / `chunk_seam_profile` / `chunk_porosity` / `chunk_s2` / `chunk_profile` resolve the same seams PER CHUNK along the generation order, on a reference chunk grid given by the assessment and never read from the volume's own manifest — each block owns the planes at its own origin, so every plane has exactly one owner and chunk 0 owns none. `chunk_bounds` + `chunk_band_profile` read the -8/+0 porosity band at the chunk frontiers — the planes MOVE with the sampler's chunk overlap, and the last plane on each axis is reported apart because its successor's far face is the volume edge, the one frontier configuration the model was trained on. `scripts/analysis/chunk_band_trial_report.py` calls it rather than keeping a second copy. |
| `eval_v4/cases.py` | The thirteen assessments as data — 231 `CaseSpec`s over seeds 101/202/303 — plus the requested-field painters (halves, checkerboard, coherent), the notch-and-hole material map, and the three layups (A and B16 from `data/layup_ground_truth.json`, C a fixed permutation of A). `CaseSpec.neighbour_mode` names where a case's neighbours come from; only `assembly_modes_cases` asks for anything but the production `"canvas"`. `MEASURE_ONLY` names the assessments eval_v4 MEASURES but never GENERATES: `field_stats` re-measures volumes another assessment wrote, and `slicegan` is a baseline whose volumes come from its own sampling script. Nothing here knows the sampler. |
...
| `eval_v4/measure.py` | One measurer per assessment, aggregating mean ± sd over seeds (`measure_microstructure` instead reports every distance three ways — generated vs real, the real-vs-real floor, and the ratio — and carries the `memorisation` block, which is measured on the `sampler` and `porosity_global` volumes rather than on its own); plus `manifest_check`, which separates a manifest that does not parse, a volume that contradicts its manifest, and a case the assessment defines but the campaign lacks. Never re-reads the model. `measure_field_stats` is the one measure-only measurer: it reads the coherent-field volumes `porosity_local` and `multichunk` wrote plus the real crops, so `n_cases_expected` comes from what it found and not from a case list.  `measure_ood_conditioning` reports, per case, what the model was ACTUALLY conditioned on and whether the training-range clamp touched it — a row at phi 0.20 would otherwise be indistinguishable from one at 0.107, which is what the clamp would have made of it. `measure_stress_geometry` runs the gated assessments' own metrics on shapes the model was never trained to make, and every block it cannot produce says so with a reason instead of raising: a `full` request has no requested air to score, and layup recovery needs the 1024-voxel T-I window in BOTH in-plane axes, which four of the nine requests are too narrow for (`taper` was widened to 1024 in x so that it is not; the L-bracket cannot be fixed that way, because each leg is only 224 voxels long once the corner is excluded). `measure_slicegan` scores the SliceGAN baseline on the request-free metrics only, against all three matched real levels rather than the one nearest its delivered porosity — the generator is unconditional, so the porosity cannot be matched and a single chosen level would let the choice carry the result. Its `_anisotropy_block` reads directional S2 and adjacent-slice contrast per axis for generated AND real, because real laminate is anisotropic and a generated anisotropy is only a fault when it differs from the real one. `measure_microstructure` is the only measurer that takes `allow_busy_gpu`, because it is the only one carrying the full-store memorisation search: that search skips itself with a reason when another CUDA job holds the card, and this overrides it. |
| `eval_v4/microstructure.py` | The four distribution statistics, ported from the deleted `scripts/eval_generated_volumes.py` and each validated against a structure whose answer is known: S2(r) by Hann-windowed FFT autocorrelation debiased with the window's own autocorrelation, the pore-size distribution of 6-connected components, border-corrected Ripley's K over a KD-tree (converges to the CSR `(4/3)pi r^3`, unlike the uncorrected estimator it replaces), and FID on 64x64 native slice crops through torchvision's InceptionV3 pool3. `compare_sets` is the two-sample comparison the assessment runs twice — generated against real, and real against real. `s2_directional` is the SAME estimator binned by direction instead of by radius, because the radial curve averages over direction and so cannot tell a pore that is flat in z from one that is long in z — two opposite anisotropies it orders only by mean pore size (`tests/test_eval_v4_microstructure.py` states that as a test). `s2_directional_profile` adds the one-number anisotropy it is read by. A missing torchvision is a reported skip, not a failure. The memorisation check is NOT here; it is `eval_v4/memorisation.py`. |
| `eval_v4/memorisation.py` | The fifth statistic of assessment 8, and the only one that is not a distribution distance: a FULL-STORE nearest-neighbour search. Every 64-cubed whole-material patch of every generated volume of at most `MAX_TILES_PER_VOLUME` tiles in `sampler`, `porosity_global`, `multichunk` and `assembly_modes` is searched against ALL 219 580 stride-64 rows of the train split (not a sample — the earlier 10 000-row sample gave an upper bound on the distance, which is the wrong side of the question), in BOTH latent space (re-encoded through the frozen VAE, per-channel normalised with the store's own stats) and decoded grey space (against the raw `patches_xct.bin` source patches the store was built from). The statistic is Favero's `||x-x'|| / ||x-x''||` with the <1/3 memorisation verdict. Grey carries TWO floors: `grey` is the val patch ROUND TRIPPED through the same frozen VAE, so it has the decoder error the generated query has and the comparison is like-for-like; `grey_raw` is the untouched scan patch and bounds it from the other side. `Top2` streams the bank a chunk at a time (the store is 272 GB); `squared_distances` only RANKS, and `refine` recomputes the two chosen distances in float64 so the chunk width cannot reach the number. Broken down by requested porosity and by `WindowStates`, which rebuilds EVERY WINDOW's EXISTS/OOB/UNKNOWN faces from the manifest's `volume_shape`, `chunk_tiles` and `window_stride` using the sampler's own window grid, chunk partition and chunk ORDER — a face is UNKNOWN only when the chunk it reaches into was not solved yet, so the same position in the last chunk is not. The 192-cubed volumes are one chunk and fill only the `present` bucket; the multichunk volumes fill both. The store comes from the manifest and nowhere else.  The search DROPS ITS OWN PAGE CACHE: every bank chunk is released with `madvise(MADV_DONTNEED)` then `posix_fadvise(POSIX_FADV_DONTNEED)` once it has been scored, so the resident file-backed set stays bounded by the chunk instead of growing to the size of the store. This machine shares 121 GB between CPU and GPU and a CUDA allocation does not wait for the kernel to reclaim page cache — it fails — while `free` still reports tens of GB available, so the symptom never points at the cause. |
| `eval_v4/field_stats.py` | The spatial statistics of the DELIVERED local porosity field — the quantity a competing paper claims a porosity field is a sufficient descriptor of (Naiff, Ramos, Wang, SSRN 10.2139/ssrn.7161201). `delivered_field` reads pore-over-material per 64-voxel window every 32 voxels out of the LABEL (never out of the request beside it), built from stride-grid block sums so a 1024-cubed crop costs megabytes and not a summed-area gigabyte; `marginal_stats`, `axis_correlations` (1/e length PER AXIS — the laminate is anisotropic, z is not x or y) and `marginal_distance` (W1 raw and mean-normalised, KS). `corr_length_gap` refuses to compare two lengths found over different reaches. `COHERENT_SOURCES` / `REAL_SHAPE_TAGS` name the volumes it reads. |
| `eval_v4/real_floor.py` | Cuts crops of the split_v3 TEST panels at the generated shapes and runs every request-free metric on them. Reduces the crop depth a tile at a time until a box fits entirely inside `sample_mask` (a 192-deep one does not exist on a real laminate), never below 128. `--shapes micro` additionally cuts the matched-porosity reference PAIRS assessment 8 is floored against: per level and per PANEL, two 128-cubed crops at that porosity that share no material. Candidates are scored exactly from a one-pass cell summary of the pore mask, so no candidate needs the volume read for it. Its surface correlation length is `metrics.lag_correlation` + `correlation_length_1_over_e`, the same pair `field_stats` reads a porosity field with. |
...
| `eval_v4/stress_geometry.py` | The builders for assessment 12: a tube, an L-bracket with a filleted corner, a linear taper, two disconnected coupons, a hollow sphere, a gyroid, cut-through letters, and the ramp-plus-spots porosity field. Separate from `cases.py` because each one has a CLOSED-FORM volume a test can check it against, which is the only way to know a voxel mask is the shape it claims to be (`tests/test_stress_geometry.py`, 27 tests against analytic volumes and not against recorded numbers). The gyroid's level is calibrated from the measured `|grad f|` rather than assumed, and `material_letters` WRAPS text that does not fit rather than shrinking it — the glyph height is the feature size the request is about. `l_bracket_legs` crops the two legs apart with the corner in neither, because no ply orientation is defined through a corner. |
| `eval_v4/report.py` | `findings.md` + figures (PDF and PNG, 300 dpi) from `results.json` alone, with the real-floor row first in every table that has one. Touches no volume. `report_stress_geometry` tables the requests in the DESIGNED order — nearest the training coupons first — because read that way the table says where the conditioning stops, and it draws its montage by importing `eval_v4_inspection_pack.render_case` rather than a second renderer that would drift from the first. |
| `models/diffusion/factory.py` | `build_denoiser(cfg)` — the one place `model.type` becomes a class. Three call sites built a denoiser and each hard-coded `UNet3DDenoiser`, so a second architecture had to be threaded through all three by hand: three chances for a generation run to build a different network than the one that was trained. An unknown type is refused rather than falling back to the production network, because a typo that silently trained ldm06's architecture under a baseline's name would produce a result indistinguishable from a real one. |
| `eval_v4/cases.py` (campaign 24) | `ablation_cases` — one conditioning input disturbed at a time, each arm read on the assessment that READS it so every number is a difference from an unablated campaign-18 row. Two kinds: a SWAP feeds a VALID input belonging to somewhere else (layup C's profile while A is scored; `_shifted_box`'s depth and distances) and measures how much the model uses the signal; a ZERO feeds a value that cannot exist ((cos2θ,sin2θ)=(0,0) is off the unit circle, six face distances of zero mean a window touches all six faces at once) and measures the response to nonsense. `_shifted_box` EXTENDS the box rather than moving it: a same-sized box moved by +dz puts the canvas outside it, the distances then saturate to what the unshifted box already gives and cond_depth clips to 0.0 — which is the zeroed row, so the swap would have silently measured nothing. The neighbour arm is absent on purpose; all-UNKNOWN is `assembly_modes`' `joint`, already measured at both scales. |
| `eval_v4/cli.py` | `eval_v4 generate \| measure \| report \| manifest-check \| real-floor`. `--joint` denoises each case's WHOLE canvas as one chunk with every neighbour UNKNOWN, for the porosity-only baseline, which has no neighbour conditioning to chunk for. `measure` accepts the MEASURE_ONLY names too, which is how a baseline reaches the same tables. |
...
| Path | What it does |
|---|---|
| `train_slicegan.py` | Trains the SliceGAN baseline into `runs/campaigns/22-slicegan-baseline/train/`. Builds the slice bank ONCE and then never reads the store again. |
| `train_ddpm3d.py` | Trains the pixel-space DDPM baseline into `runs/campaigns/23-ddpm3d-baseline/train/`, under a wall-clock cap. |
...
| `analysis/slicegan_sample.py` | Samples the trained SliceGAN into eval_v4's own case layout, so the baseline meets the same tables. The latent is TILED WITH A HALO: a 192x1024x1024 volume in one forward pass took this machine to 119 GB of its 121 GB unified pool. The generator is fully convolutional, so a tile with a 2-cell halo reproduces the single pass BIT-EXACTLY — the halo is measured, not guessed, and `tests/test_slicegan.py` asserts the reproduction and that a 1-cell halo would not give it. Only `n - 2` of a latent's `n` cells produce output; tiling all `n` asks for output past the end of the volume, which the guard caught rather than silently truncating. The manifests carry NO requests — SliceGAN cannot be asked for anything — while `requested_material="full"`, `chunk_tiles` and `window_stride` are MEASUREMENT CONVENTIONS carried so the seam statistic is evaluated at the same planes ldm06 is scored at. |
| `analysis/ddpm3d_sample.py` | The same, for the pixel-space DDPM. |
| `analysis/downstream_utility.py` (segmenters) | `SegUNet3D` is campaign 14's own plain 3-level U-Net (1.849 M) and stays the default; `SegResUNet3D` (2.574 M) is the same depth, widths, skips and head with residual blocks, selected by `--architecture resunet`. It exists because campaign 14's conclusion — that our synthetic data HURTS a segmenter — was measured through one network, and the cheapest way for that to be an artefact is an inductive bias that happened to suit real data. It is NOT parameter-matched (39 % more at the same base width) and says so: narrowing it to match would trade a capacity confound for a width one. The architecture is a `Budget` field, so it cannot vary between arms of one comparison and lands in the results beside every number it produced. |
| `analysis/slicegan_inspection.py` | Mid-slice PNGs of the baseline through `eval_v4_inspection_pack.render_case` — the SAME renderer the ldm06 pack uses, so a baseline panel and an ldm06 panel differ in content and not in plotting script. |
...
`r08_calibration_probe`) · `15-sample-mask-audit`
...
| `analysis/memorisation_smoke.py` | Proves the memorisation hardware path — the VAE load, the encode, `decode_grey` and both full-store passes — on ONE 192-cubed volume and ONE 384-cubed multichunk volume before the full search is given the GPU for hours. The two shapes differ in the thing that needed testing: a 192-cubed volume is one chunk and fills only the `present` neighbour bucket, a 384-cubed one fills the UNKNOWN bucket too. The numbers it prints answer nothing about memorisation — two volumes against the full bank is not the assessment — and it says so. The cap is enforced from OUTSIDE, by a parent that polls the child's host RSS and GPU memory: a watchdog inside the child shares the child's fate, and an allocation that runs away would take the reporter with it.  The cap is on ANONYMOUS resident memory, not VmRSS: `PatchStore` memory-maps a 195 GiB store, so a healthy streaming pass leaves gigabytes of clean page-cache resident and charged to VmRSS.  The first version capped on VmRSS and killed the first real run at 22.2 GB after 2.9 min with the GPU at 5.1 GB and nothing wrong.  It REFUSES to start while another CUDA job holds the card, for the same unified-memory reason; the guard is in the script and not only in the queue script, because the rule has to hold for a hand-run too. |
...
| `analysis/build_eval_v4_notebook.py` | Generates the eval-v4 inspection notebook (`runs/campaigns/12-eval-v4/inspect_eval_v4.ipynb`, its `requirements-inspect.txt`, and the committed copy `notebooks/eval_v4_inspection.ipynb`). Every cell is a string in this file so the notebook the author runs on a laptop is traceable to a commit. Self-contained (json / tifffile / plotly / ipywidgets; poregen optional and guarded); every section is guarded so it executes end to end on a partial campaign. Needs `nbformat` (in the `notebook` extra). |
| `analysis/vae_recon_figure.py` | Side-by-side reconstructions of several VAE checkpoints on the harness's deterministic held-out patches (shares `vae_val_l1.load_model_and_patches`), central slice per patch with its L1 and its texture correlation (recon vs input after removing the patch mean). Written because campaign 28's L1 table hid that the flat VRRAEs reconstruct the mean grey and the air boundary but almost no texture. CPU. |
| `analysis/side_by_side_real_vs_generated.py` | Side-by-side figures of a real test crop vs a generated volume at matched porosity and layup (three orthogonal mid-slices, grey and label overlay), plus a full-width 1024 comparison and a through-thickness strip; writes `<campaign>/figures_side_by_side/`. CPU only. |
...
| `analysis/chunk_band_latents.py` | Is the chunk-plane band a BLUR or a CONTENT effect? (CPU, read-only.) Reads the saved `latents.npy` and `probs.npz` of volumes that already exist. A blur — the fusion averaging two disagreeing predictions, whose mean is not a sparse latent — would DROP the latent std across the band and spread pore probability over more voxels at a lower peak; a content effect leaves the std normal and the probability simply low. Everything is a ratio to the volume's own interior, because the arms differ in overall porosity, with the store's sampled latents as the reference scale. On ldm06 the std stays within 2.7 % of interior while pore probability falls 5x: content, not blur. |
| `analysis/chunk_band_trial.py` | Does anything remove the band? (GPU.) Regenerates the hybrid arm at 1024 and 384 under four candidates — s_nb 0.5, chunk overlap 32 blended, both, and overlap PINNED-only (kept to show that pinning alone does not work) — through the production `VolumeRunner`/`CaseSpec` path, so every trial volume carries a manifest naming the settings that made it and cannot be mistaken for a production one. Refuses to start while another CUDA job holds the card. |
| `analysis/chunk_band_trial_report.py` | Scores the trial arms against the success criterion and prints them beside the campaign-12 hybrid row they are meant to replace, because "better" only means better than what we have. Criterion: the -8 AND +0 slab phi within 20 % of the volume mean (the depletion sits on BOTH single-covered strips, so clearing one is not clearing the band), the grey chunk seam still at the real floor, and delivered phi inside the 0.005 gate. Validated against the baseline, which it correctly fails on the band (-8/mean 0.317, +0/mean 0.730) while passing phi and grey. |
| `analysis/chunk_plane_profile.py` | What is actually different at a chunk plane? (CPU, read-only.) Two eval-v4 numbers said something was wrong there and neither said what: `seam_chunk_pore_ratio` is BELOW 1, which means the field changes LESS across the plane than inside a chunk, and `pore_dice_across_chunk_planes` is a bare adjacent-slice Dice with no scale. This supplies both missing controls — phi in 8-voxel slabs stepping across each plane, and the adjacent-slice Dice at interior planes of the same volume. Run on ldm06 it found the cause: phi collapses from ~0.028 to ~0.004 in the last 16 voxels before EVERY chunk plane and recovers over ~24 after, while a real 1024-wide crop is flat (0.005) across the same planes. One defect, not two — there are barely any pores at the plane, so both numbers follow. |
| `analysis/window_rim_test.py` | Does the model under-predict pores in the RIM of a window that has a neighbour? (GPU, ~15 min.) A canvas of real val latents, 6x6x6 tiles, each tile its own chunk AND its own single window (`chunk_tiles=(1,1,1)`, `window_stride=64`) so NOTHING overlaps and no fusion can average a rim bias away; `neighbour_mode="reference"` makes every in-bounds face EXISTS, so the 4x4x4 interior tiles are windows with six real neighbours. Reports phi by distance from the window face at s_nb 1 and 0, against the same canvas decoded without denoising. `--neighbours <latents.npy>` swaps real neighbours for generated ones. Refuses to start while another CUDA job holds the card. |
| `analysis/real_porosity_by_region.py` | What porosity the 80 REAL volumes deliver, whole and by region. (CPU, seconds, index only.) Reads `patch_index.parquet` and the per-volume shapes in `index_report.json` and NEVER opens the zarr — a training run holds the card and the page cache. The index carries no voxel counts, only `porosity` and `air_fraction` as fractions of 64³; the counts come back EXACTLY, because 64³ is 2¹⁸ and a count/2¹⁸ is a dyadic rational inside float32's 24-bit mantissa. Uses the NON-OVERLAPPING stride-64 partition (14.0 % of the stride-32 rows): summing all of them would weight the interior eight times and the border once. Regions are thirds of the TRUE volume shape by patch centre, which on the 185-332-voxel z axis leaves 10 volumes with an empty back third — the arithmetic is reported and a per-64-voxel-slab z profile is added, because a slab means the same thing in every volume and a third does not. Writes `runs/campaigns/21-real-porosity-by-region/`. |
| `analysis/decoder_ft_redecode.py` (selection) | `--ddim-steps` filters the globbed canvases by each case's MANIFEST `ddim_steps`, never by the directory name. Case names carry the step count only where the assessment varies it — on campaign 12 `*ddim200*` matches 45 of the 99 DDIM-200 canvases — so a name glob builds a gate row out of half its volumes and reports it as whole. A canvas whose manifest is missing or unreadable is dropped and named, because the gate compares two decoders on IDENTICAL input. |
...
| `ldm06_post.sh` | Waits for ldm06/base to reach `total_steps` (not merely for the process to vanish — a deliberate two-minute stop is not the end of training), refreshes the latent std reference, then runs the gated eval-v4 GENERATION and stops. The gate is `runs/campaigns/ldm06_go`, read ONCE when ldm06 exits so a late touch cannot retro-insert hours of generation ahead of the fine-tune. Everything after generation — the pack, the measure/report, the decoder-ft gate, the re-decode, the owed rung reports and the bracket rungs — belongs to `ldm06_post_tail.sh`, which stops this script and takes the queue. Two scripts that both launch `r08/decoder-ft` would run the fine-tune twice. |
| `ldm06_post_tail.sh` | Owns the eval-v4 queue outright: it stops `ldm06_post.sh`, lets that runner's in-flight `generate` finish (killing it would only make the case start over — `generate` skips any case that already has a `manifest.json`, and `save_case` writes the manifest LAST), and runs the rest in an order the runner's fixed stage list cannot express. layup is 48 % of the remaining generation, so it moves to the END and the inspection pack is built before it. `label_uncertainty` starts at once on the CPU beside the generation; `field_stats` is measured only after `multichunk` has written the volumes it re-reads; `assembly_modes` is generated before the measure stage, because the full memorisation pass that rides inside `measure microstructure` searches its volumes; `downstream_utility` is last of all and is preempted by the decoder fine-tune when the go-ahead is already there. The memorisation smoke test and `measure microstructure` both run under a wall-clock and memory cap, so a search that runs away cannot hold the card. |
| `final_queue.sh` | THE ONE QUEUE: everything the card does from the end of the facedrop trial to the final notebook, in the author's order — campaign 18 regeneration, stress DDIM-50, ood_conditioning, the decoder fine-tune and the D43 gate table, downstream utility, stress DDIM-200, the r08 rf-2/rf-64 rungs, and the notebook rebuilt into 18 and 12. Waits on `runs/campaigns/regen_go` (CKPT_RUN + SAMPLER) and, for the decoder stage only, on `decoder_ft_go` — it never creates either. The decoder fine-tune lives here and not in `facedrop_post.sh` because the D43 gate must be read on the CAMPAIGN 18 volumes, which do not exist until stage 2; in the old place it would have started the moment the gate file appeared, hours early. Stage 6 deliberately uses the ORIGINAL r08 decoder: the redecode is a comparison until its gates are read. |
| `stop_fdpost_after_rim.sh` | Stops `facedrop_post.sh` at the rim-test boundary. An edit could not do it: that script was already running, and a rewrite gets a new inode while the running bash keeps reading the old one — so the running instance would still have reached its decoder-ft stage as soon as the gate file appeared. Resolves the PID and VERIFIES it against `/proc/<pid>/cmdline` before signalling, and excludes itself: a pattern kill here matches the watcher and the shell that launched it, which has cost this project two aborted runs. |
| `run_stress_geometry.sh` | Campaign 19, the stress geometries, parameterised by `CKPT_RUN`, `SAMPLER` and `PASS` (`ddim50` or `ddim200` — four times the sampling work, so it runs later in the queue). The case list comes from the case builder and never from a list typed in the script. Generates ONE case per call so a request the sampler cannot hold costs that request and not the eight after it. `cube1024_ddim200` is skipped under `a2s` (2.05x x 4x on a 1024-cubed canvas), the skip lives in the script rather than in the case list, and no `--save-latents`: the decoder-ft gate reads campaign 18, not this one. |
| `regen_eval_v4.sh` | Regenerates the WHOLE eval-v4 campaign into a new directory, parameterised by `CKPT_RUN` (which weights) and `SAMPLER` (`production` or `a2s`). Campaign 12 is never written to — it is the record of what 119k produced under the production sampler. The real floor is SYMLINKED from campaign 12 rather than recut: its inputs are the test panels and the campaign-04 detector calibration, neither of which depends on the checkpoint or the sampler, so the link is what says the two campaigns share one floor. layup runs last and the inspection pack is built before it; `field_stats` is measured from `MEASURERS` because it has no generate verb, and an assessment with NO volumes in the campaign is skipped with a reason rather than raising — `stress_geometry` and `ood_conditioning` belong to campaigns 19 and 20, and measuring them here put two meaningless rc=1 lines in the log; `microstructure` carries the full memorisation pass and runs capped with the card idle. |
| `chain.sh` | THE ONE CHAIN from the SliceGAN training exit to the final notebook: generate/measure/report/inspect the SliceGAN baseline, its two downstream arms, then the DDPM baseline end to end, then the rf-2 resume and rf-64, then the notebooks. Its exit test is RUN PROGRESS and never process absence — a run paused for two minutes is not a finished run — and no store-heavy stage runs beside a training stage, because that deadlocked rf-2 at step 22999 and cost 999 steps. |
| `chain_tail.sh` | Takes the queue from `chain.sh` after the DDPM training, in the order the supervisor decided: the owed SliceGAN FID first (minutes, and the only moment the card is idle), then DDPM generation/measure/report, then ALL FOUR baseline downstream arms in ONE invocation with nothing beside them, then rf-2, rf-64 and the notebooks. It exists rather than an edit because `chain.sh` was already running and bash reads a script by file offset. Stopping that script's SHELL leaves its `train_ddpm3d.py` child reparented to init and still training, which is what this waits on — by PID verified against `/proc/<pid>/cmdline`, then by the completion line, because a vanished process that never logged one is a crash. |
...
| `vrrae/conv_k8*.yaml` | **current VRRAE arm** | The PER-CELL bottleneck (`v2.vrrae_conv`) at r08's exact latent budget, 8 coefficients x 16^3 cells. The suffix is the KL weight and it is the ONLY difference between the three: `conv_k8` at 0.025 keeps 0.02 nats per coefficient and reconstructs smoothly; `conv_k8_b1e-5` keeps 2.9 and beats r08 while doing it, which is autoencoding rather than a posterior; `conv_k8_b1e-3` is the untried arm between them, aimed at r08's own 0.26-0.37 per channel so the comparison is like-for-like in regularisation as well as in budget. |
...
| `test_memorisation_smoke.py` | The smoke test's memory cap, on `/proc` data whose answer is known: that memory-mapped store pages do NOT count towards the cap (the exact shape of the run that was wrongly killed — all file-backed, zero anonymous), that anonymous and shared memory DO, that a child which exits mid-poll reads zero rather than raising, and that the smoke pair is one single-chunk and one multi-chunk volume so both neighbour buckets are exercised. |
| `test_window_rim_test.py` | The rim statistic on volumes whose answer is planted: that a uniform porosity is FLAT across every shell (a porosity difference must not read as a rim effect), that a planted pore-free outer 16 voxels shows in the first two shells only, that the shells partition the tile exactly, that only the 64 interior windows are pooled (filling the boundary tiles alone leaves the profile at zero), and that air is out of the denominator. |
...
| `docs/eval_methodology.md` | eval v4: the manifest contract, every metric, the eleven assessments and the real-volume floor — the paper methods reference. |
...
└── campaigns/                    analysis output — ONE CAMPAIGN PER QUESTION
    ...
    ├── 22-slicegan-baseline/     train/ (checkpoints + log), slicegan/volumes/,
    │                             slicegan/{results.json,findings.md,figures/},
    │                             inspection/, generation.json, README.md.
    │                             real_floor/ is SYMLINKED to campaign 12's: the
    │                             floor is the test panels and the detector, and
    │                             depends on no model, so the link is what says
    │                             the campaigns share one floor.
    └── 23-ddpm3d-baseline/       the same shape, for the pixel-space DDPM
```
...
| `notebooks/` | Training / eval / R03 analysis notebooks, plus `eval_v4_inspection.ipynb` — the committed copy of the generated eval-v4 inspection notebook (regenerate with `scripts/analysis/build_eval_v4_notebook.py`; do not hand-edit). |
