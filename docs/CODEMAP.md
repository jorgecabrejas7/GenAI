...
| `eval_v4/memorisation.py` | The fifth statistic of assessment 8, and the only one that is not a distribution distance: a FULL-STORE nearest-neighbour search. Every 64-cubed whole-material patch of every 192-cubed `sampler` and `porosity_global` volume is searched against ALL ~219 580 stride-64 rows of the train split (not a sample — the earlier 10 000-row sample gave an upper bound on the distance, which is the wrong side of the question), in BOTH latent space (re-encoded through the frozen VAE, per-channel normalised with the store's own stats) and decoded grey space (against the raw `patches_xct.bin` source patches the store was built from). The statistic is Favero's `||x-x'|| / ||x-x''||` with the <1/3 memorisation verdict; the floor is the same ratio for real VALIDATION patches against the same bank, without which the generated number means nothing. `Top2` streams the bank a chunk at a time (the store is 272 GB); `squared_distances` only RANKS, and `refine` recomputes the two chosen distances in float64 so the chunk width cannot reach the number. Broken down by requested porosity and by `neighbour_census`, which rebuilds each volume's EXISTS/OOB/UNKNOWN window-face counts from the manifest's `volume_shape`, `chunk_tiles` and `window_stride` using the sampler's own rule. The store comes from the manifest and nowhere else. |
