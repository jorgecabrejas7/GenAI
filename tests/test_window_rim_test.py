...
def canvas(fill=MATERIAL) -> np.ndarray:
    return np.full(W.SHAPE, fill, np.uint8)
...
def per_tile(fn) -> np.ndarray:
    ...
    for tz in range(W.TILES_PER_AXIS):
        for ty in range(W.TILES_PER_AXIS):
            for tx in range(W.TILES_PER_AXIS):
...
class TestRimProfile:
    ...
    def test_uniform_porosity_is_flat_across_every_shell(self):
        ...
        lab = np.where(rng.random(W.SHAPE) < 0.25, PORE, MATERIAL).astype(np.uint8)
        prof = W.rim_profile(lab)
    ...
    def test_a_planted_rim_shows_in_the_first_shells_only(self):
        ...
        prof = W.rim_profile(per_tile(pattern))
    ...
    def test_the_shells_partition_the_tile_exactly(self):
        ...
        assert counted == n ** 3
        assert W.interior_phi(lab) == pytest.approx(1.0)
        assert t ** 3 == W.TILES_PER_AXIS_INTERIOR_COUNT
    ...
    def test_only_the_interior_windows_are_pooled(self):
        ...
        """
        lab = canvas()
        n, T = W.TILE, W.TILES_PER_AXIS
        for tz in range(T):
            for ty in range(T):
                for tx in range(T):
                    if 0 < tz < T - 1 and 0 < ty < T - 1 and 0 < tx < T - 1:
                        continue
                    lab[tz * n:(tz + 1) * n, ty * n:(ty + 1) * n, tx * n:(tx + 1) * n] = PORE
        assert W.rim_profile(lab) == {k: pytest.approx(0.0) for k in ("0", "8", "16", "24")}
        assert W.interior_phi(lab) == pytest.approx(0.0)
