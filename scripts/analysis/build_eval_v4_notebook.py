...
**Two seam columns.** The plain chunk seam compares every adjacent slice pair across a chunk plane, including air–air
pairs. When a requested surface lies exactly on a chunk plane (the L-bracket's leg thickness equals the chunk size; the
two-coupon gap edge sits at 3×192) the plain value reads as a huge seam (1.85, 1.95) that is really the specimen edge.
The *material only* column keeps just the voxels that are material in both slices, so it answers "is the assembly
continuous inside the part": those two drop to 1.01 and 1.06 while the other shapes do not move. For any case with air in
the canvas the paper quotes the material-only column; the plain one stays for comparability with campaign 12.
""")
    code(r'''
if SR.exists():
    D = load_json(SR); rows = []
...
| 09-13 21:44 → 09-14 00:30 | **Decoder fine-tune (D43) — negative result.** r08 run-0007 early-stopped at 6360/12000 on held-out reconstruction loss (the intended criterion). Gate 1 (reconstruction sharpness ≥ 0.95 of real) 0.614 → 0.643 FAIL; gate 3 (segmentation unchanged) porosity MAE +78 %, pore Dice 0.942 → 0.928 FAIL. **The paper's decoder stays the original r08.** Downstream utility runs on the original decoder since 23:02. | section *decoder fine-tune*; vault E13, D45 |
| 09-14 19:00 | **Withdrawn:** the "sharpness mirror" (generated samples 38 % sharper than real; 21 % air disagreement between decoders; gate 2; the texture guards). The redecode decoded the saved latents without restoring their native scale; found while building the visual comparison. Val-arm gates and the rejection stand. Generated arm re-run queued after the downstream arms, with a regression test against the stored volumes. Refiner decision deferred to the corrected number. | vault E13, D45 |
...
| 09-16 20:33 → 09-17 | **SliceGAN baseline trained (60k steps, 11 h), generated and measured.** Unconditional porosity 0.016–0.032; **no air at all**; anisotropy equal to real (directional S2 0.84 vs real 1.23 ± 0.43; per-axis contrast and seams match), so the late per-axis critic divergence is real anisotropy, not collapse. Microstructure vs floor at the nearest real level (0.03): S2 2.6, PSD 1.5, Ripley 1.1 (ours: 2.2 / 1.5 / 0.73). Seams on ldm06's plane grid all ≈ 1 (control). Downstream arms lost to a dataloader SIGBUS when a generation ran beside them (store-contention rule, second instance); rescheduled with the DDPM arms. DDPM baseline training since ~00:30, 24 h cap. | section *baselines*; vault E15 |
...
| 09-21 03:18 | **Downstream table complete (14 arms + a second segmenter).** Synthetic-only pore Dice on real scans: ours 0.514, pixel DDPM 0.542, φ-only 0.546, SliceGAN 0.151 (no air ever seen); as augmentation: real+DDPM 0.841 (no harm), real+φ-only 0.832, real+ours 0.815, real+SliceGAN 0.816; ResUNet: real 0.876, ours 0.558, SliceGAN 0.212. Readings: the transfer gap is a property of synthetic CT from every diffusion generator here, not of our model; ours is not better training data than the unconditional DDPM and costs more as augmentation (pool composition is a plausible, unproven cause); the conclusion survives a second architecture. rf-2 resumed from step 22 000 at 03:18. | section *downstream utility*; vault E14 |
| 09-22 → 09-23 | Fix stage done: 28 stale field cases archived and regenerated on the corrected pipeline, re-measured, re-reported; the missing assembly cell `chunked_no_neighbours` generated and in the report (φ 0.0283 at 1024); DDPM baseline measured (φ 0.004–0.006 uncontrolled, seams 1.08–1.23); **matched control (campaign 29): a 15k-step fine-tune identical to facedrop but without per-face dropout leaves the band untouched (−8 slab 0.187 vs 0.192 untrained; facedrop 1.035) — the causal claim holds.** rf-2 early-stopped at step 36 359 (patience on val.total), rc=0. VRRAE block failed overnight: V0 died at step 6636 in a TensorBoard histogram call (KL tensor empty at β = 0; resumable), the A/B memory smoke test raised before allocating (false "does not fit"), B's fallback at batch 1024 pushed the host out of memory at 02:29 and the kernel killed tmux; workers halved, smoke test fixed, chain now stops after two failures. rf-64 started 09:29 and is being paused for the VRRAE block. | section *chunk-plane band* (control row), *baselines*; vault E8 §matched control, log |
...
**Result (2026-09-14): a negative result.** The fine-tune (r08 run-0007, encoder frozen, early-stopped at 6,360 of
12,000 steps on held-out reconstruction loss) fails both gates measured on real validation windows: reconstruction
sharpness reaches only 0.64 of real (gate ≥ 0.95), and segmentation degrades (porosity MAE +78 %, pore Dice 0.942 →
0.928). **The paper's decoder is the original r08 decoder.**

**Withdrawn (2026-09-14 19:00).** The first report of this campaign also claimed that generated samples were 38 % sharper
than real and that the two decoders disagreed on 21 % of air voxels. Those numbers came from the "generated" arm of the
redecode, which decoded the saved latents in the diffusion model's normalised space without restoring the native scale
(a check against the stored volumes showed a 17 grey-level error instead of 2). Every generated-arm number (gate 2, the
S2/PSD guards, the pore/air agreement) was withdrawn and re-measured with the scaling fixed. The rejection did not
depend on them: both failing gates are on the validation arm, which was decoded correctly.

**Corrected generated arm (2026-09-15).** With the paper's decoder, generated volumes are **0.84 (DDIM-50) / 0.86 (DDIM-200)
of real sharpness**: duller than real, not sharper. The fine-tuned decoder brings them to 0.90 / 0.92, the right direction
but still below the 0.95 gate, and it still over-calls pores (+5 %) with the segmentation cost seen on real windows. The two
decoders agree on pores at 0.95 and on air at 0.89–0.91. Verdict unchanged: rejected; the original decoder is the paper's.
A refiner is now legitimate future work (samples are duller than real), not part of this paper.
""")
    code(r'''
