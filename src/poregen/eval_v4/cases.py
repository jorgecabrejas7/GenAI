...
def assembly_modes_cases(repo=None) -> list[CaseSpec]:
    ...
    for shape, tag in ASSEMBLY_MODES_SHAPES:
        ...
        arms = [
            ...
            # THE MISSING CELL OF THE FACTORIAL. `joint` varies chunking AND
            # neighbour content together, and `hybrid` has both; neither
            # separates them. This is production CHUNKING with the neighbour
            # CONTENT removed — same chunk grid as hybrid, neighbours all
            # UNKNOWN — so hybrid-minus-this is what the neighbour content
            # buys, and this-minus-joint is what chunking costs on its own.
            ("chunked_no_neighbours", CHUNK_TILES, "unknown"),
        ]
...
#: Measurable but NOT generatable by eval_v4. ``field_stats`` re-reads what
#: porosity_local and multichunk wrote. The baselines are generated by their own
#: sampling scripts — eval_v4 has no model for them — and are measured here so
#: the baseline and ldm06 meet the same tables rather than two different ones.
MEASURE_ONLY = ("field_stats", "slicegan", "ddpm3d")
